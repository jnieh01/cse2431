...
def student_producer(producer_num, f_in, buffer, locks):
    ...
    while not buffer.KILL:    
        ...
        while ((buffer.IN + 1) % buffer.NUM_SLOTS) == buffer.OUT :        # Buffer full, wait consumer
            pass
        ...
        buffer.IN = (buffer.IN + 1) % buffer.NUM_SLOTS                     # Produce item
...
def student_consumer(consumer_num, f_out, buffer, locks):
    ...
    while not buffer.KILL:
        ...
        while buffer.IN == buffer.OUT:                                             # Buffer empty, consumer wait
            if buffer.PRODUCERS_DONE and buffer.IN == buffer.OUT:                  # Buffer empty and producer finsihed, then consumer finished
                locks.consumer_buffer.release()
                return
        ...
        buffer.OUT = (buffer.OUT + 1) %  buffer.NUM_SLOTS                          # Consume item      
        locks.consumer_buffer.release()
